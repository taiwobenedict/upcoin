...
function Calendar() {
  ...
  const getStatisticStringOfADay = (day) => {
    ...
    dayStakings.map((staking) => {
      ...
      if (stat) {
        ...
        let newAmount = parseFloat(stat._amount) + parseFloat(formatEther(staking._amount));
        ...
        statistics.push({
          _poolIndex: staking._poolIndex,
          _amount: parseFloat(formatEther(staking._amount))
        });
      }
  ...
  return (
    <div className="calendar h-full grid grid-rows-7 border-[2px] rounded-lg border-gray-500">
      ...
      <div className='calendar-weeks w-full row '>
        {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map((day, index) => (
          <div key={day} className={`col d-flex align-items-center justify-content-center  text-14 font-semibold  
                ${index % 7 === 0 ? "day-cell border-t-2 border-r-2   border-gray-500 " :
              index % 7 === 6 ? "day-cell border-t-2  border-gray-500  " :
                "day-cell border-t-2 border-r-2  border-gray-500"}
                  `}
      ...
      <div class="container">
        <div className="calendar-days w-full row">
          {days.map((day, index) => {
            ...
            return (
              <div key={index}>
                ...
                {/* <div className={
                `d-flex justify-content-start align-items-start text-18 font-semibold p-3
              ...
              >
                ...
                {
                  index % 7 == 6 && <div className='w-100'></div>
                }
              </div>
            )
          })}
      ...
      >
        <div className="d-flex flex-column min-w-[200px] min-h-[100px] align-items-center pb-5">
          <div className='mt-3 text-[18px] font-semibold'>{selectedDate}</div>
          {
            dayStatistics?.length > 0 && dayStatistics.map((item, index) => (
              <div key={index} className='mt-3 text-[16px] font-semibold w-full px-5 text-left'>
                {poolsData[item["_poolIndex"]]?.period} Months pool: {Math.floor(item["_amount"])} SUP</div>
            ))
          }
        </div>
      </Popup>
